...
#ifndef Peipei_CLAMP // return the clamped value
#define Peipei_CLAMP(_x_, _low_, _high_)  (((_x_) > (_high_)) ? (_high_) : (((_x_) < (_low_)) ? (_low_) : (_x_)))
#endif


@interface PeipeiPictureGroupItem()<NSCopying>
@property (nonatomic, readonly) UIImage *thumbImage;
@property (nonatomic, readonly) BOOL thumbClippedToTop;
- (BOOL)shouldClipToTop:(CGSize)imageSize forView:(UIView *)view;
@end
@implementation PeipeiPictureGroupItem
...
- (id)copyWithZone:(NSZone *)zone {
    PeipeiPictureGroupItem *item = [self.class new];
    return item;
}
@end
...
@interface PeipeiPictureGroupCell : UIScrollView <UIScrollViewDelegate>
...
@property (nonatomic, strong) PeipeiPictureGroupItem *item;
@property (nonatomic, readonly) BOOL itemDidLoad;
- (void)resizeSubviewSize;
@end

@implementation PeipeiPictureGroupCell
...
- (instancetype)init {
    ...
    //NSLog(@"groupview------%@",[NSThread currentThread]);
...
- (void)setItem:(PeipeiPictureGroupItem *)item {
...
@interface PeipeiPictureGroupView() <UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@implementation PeipeiPictureGroupView
...
- (void)doubleTap:(UITapGestureRecognizer *)g {
    ...
    PeipeiPictureGroupCell *tile = [self cellForPage:self.currentPage];
    if (tile) {
        if (tile.zoomScale > 1) {
    ...
    switch (g.state) {
        ...
        case UIGestureRecognizerStateEnded: {
            ...
            if (fabs(v.y) > 1000 || fabs(deltaY) > 120) {
                ...
                duration *= 0.8;
                duration = Peipei_CLAMP(duration, 0.05, 0.3);
...
- (void)presentFromImageView:(UIView *)fromView
    ...
    for (NSUInteger i = 0; i < self.groupItems.count; i++) {
        PeipeiPictureGroupItem *groupItem = (PeipeiPictureGroupItem *)self.groupItems[i];
        if (fromView == groupItem.thumbView) {
            page = (int)i;
            break;
        }
    }
    ...
    CGSize contentSize = CGSizeMake(_scrollView.width * self.groupItems.count, _scrollView.height);
    [_scrollView setContentSize:contentSize];
    CGRect rect = CGRectMake(_scrollView.width * _pager.currentPage, 0, _scrollView.width, _scrollView.height);
    ...
    PeipeiPictureGroupCell *cell = [self cellForPage:self.currentPage];
    PeipeiPictureGroupItem *item = _groupItems[self.currentPage];
...
- (void)dismissAnimated:(BOOL)animated completion:(void (^)(void))completion {
    ...
    PeipeiPictureGroupCell *cell = [self cellForPage:currentPage];
    PeipeiPictureGroupItem *item = _groupItems[currentPage];
    ...
    if (isFromImageClipped) {
        [self scrollToTopAnimated:NO cell:cell];
    }
...
- (void)scrollToTopAnimated:(BOOL)animated cell:(PeipeiPictureGroupCell *)cell {
...
//to cancel all request
- (void)cancelAllImageLoad {
    [_cells enumerateObjectsUsingBlock:^(PeipeiPictureGroupCell *cell, NSUInteger idx, BOOL *stop) {
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    for (NSInteger i = page - 1; i <= page + 1; i++) { // preload left and right cell
        if (i >= 0 && i < self.groupItems.count) {
            PeipeiPictureGroupCell *cell = [self cellForPage:i];
            if (!cell) {
                PeipeiPictureGroupCell *cell = [self dequeueReusableCell];
                cell.page = i;
...
- (void)updateCellsForReuse {
    for (PeipeiPictureGroupCell *cell in _cells) {
        if (cell.superview) {
...
- (void)showHUD:(NSString *)msg {
    ...
    CGSize size = [msg boundingRectWithSize:CGSizeMake(200, 200) options:NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:@{NSFontAttributeName:font} context:nil].size;
    UILabel *label = [UILabel new];
    label.size = [self CGSizePixelCeil:size];
    label.font = font;
    label.text = msg;
...
- (PeipeiPictureGroupCell *)dequeueReusableCell {
    PeipeiPictureGroupCell *cell = nil;
    for (cell in _cells) {
        if (!cell.superview) {
            return cell;
        }
    ...
    cell = [PeipeiPictureGroupCell new];
...
- (PeipeiPictureGroupCell *)cellForPage:(NSInteger)page {
    for (PeipeiPictureGroupCell *cell in _cells) {
        if (cell.page == page) {
            return cell;
        }
